...
####
###týndar tölur
joined_data$`2020`[joined_data$Reitur=="R65"]
...
for (i in unique(joined_data$Stadur)[16]) {
  ...
  #filtered_data$`2020` <- rowMeans(filtered_data[,existing_year_columns],na.rm = T)
  ...
  df <- jd_long |> 
    ...
    group_by(Type,Year) %>% #Passa ef það dettu út ár
    summarise(Mean=mean(Summa, na.rm=TRUE), .groups = 'drop')
  ...
  # tyndar
  df_with_heildarþekja[df_with_heildarþekja$Year==2020,3] <- tyndar
  
  df_with_heildarþekja  <- df_with_heildarþekja |> 
    mutate(Mean = case_when(
      Year == 2020 & Type== "Mosar" ~ 2.87,
      Year == 2020 & Type== "Blað- og runnfléttur" ~ 15.67,
      Year == 2020 & Type== "Hrúðurfléttur"~ 30.70,
      Year == 2020 & Type== "Heildarþekja" ~ 59.24,
      Year == 2020 & Type== "Tegundafjöldi" ~ 8.67,
      TRUE ~ Mean)) 
